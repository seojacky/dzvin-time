// settings-schedule-manager.js - Управління модальним вікном налаштувань розкладу (БЕЗ ПРОКСІ)
...
export class SettingsScheduleManager {
    ...
    async loadModalHTML() {
        try {
            const response = await fetch('/settings-schedule.html');
            if (!response.ok) {
    ...
    // === API МЕТОДИ (ОНОВЛЕНО - БЕЗ ПРОКСІ) ===

    // Універсальний метод для fetch запитів
    async fetchAPI(endpoint, params = {}) {
        ...
        console.log(`API запит: ${url.toString()}`);
    ...
    async loadFaculties() {
        try {
            ...
            const faculties = await this.fetchAPI('/faculties/get-all');
    ...
    async loadGroups(facultyId) {
        try {
            ...
            const groups = await this.fetchAPI('/groups/get-all-by-id-faculty', { 
                idFaculty: facultyId 
            });
    ...
    async loadCafedras(facultyId) {
        try {
            ...
            const cafedras = await this.fetchAPI('/cafedras/get-all-by-id-faculty', { 
                idFaculty: facultyId 
            });
    ...
    async loadInstructors(cafedraId) {
        try {
            ...
            const instructors = await this.fetchAPI('/instructors/get-all-by-cafedra', { 
                idCafedra: cafedraId 
            });
