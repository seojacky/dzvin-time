...
async function highlightCurrentClass() {
    ...
    scheduleItems.forEach(item => {
        ...
        if (startTime && endTime) {
            ...
            // Очищуємо всі класи та inline стилі
            ...
            if (currentTime >= startMinutes && currentTime < endMinutes) {
                if (item.classList.contains('break')) {
                    ...
                    item.classList.add('current-class', 'current');
                }
...
async function initScheduleTab() {
    ...
    try {
        // Трекінг ініціалізації розкладу
        trackEvent('Schedule Init Start');
        ...
        trackEvent('Schedule Init Complete');
    } catch (error) {
        console.error('Помилка ініціалізації розкладу:', error);
        trackEvent('Schedule Error', { type: 'init', message: error.message });
...
async function showScheduleSettings() {
    ...
    try {
        ...
        trackEvent('Schedule Error', { type: 'settings_modal', message: error.message });
        ...
        if (scheduleContent) {
            ...
            if (retryBtn) {
                retryBtn.addEventListener('click', () => {
                    trackEvent('Schedule Settings', { action: 'retry' });
...
function showScheduleError(message) {
    ...
    if (scheduleContent) {
        ...
        if (retryBtn) {
            retryBtn.addEventListener('click', () => {
                trackEvent('Schedule Error', { action: 'retry' });
        ...
        if (resetBtn) {
            resetBtn.addEventListener('click', () => {
                trackEvent('Schedule Settings', { action: 'reset' });
...
async function init() {
    try {
        ...
        trackEvent('Init Error', { message: error.message });
        ...
        button.addEventListener('click', async () => {
            ...
            if (targetTab === 'schedule') {
                try {
                    ...
                    trackEvent('Schedule Error', { type: 'tab_init', message: error.message });
