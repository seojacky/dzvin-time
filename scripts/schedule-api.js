...
export class ScheduleAPI {
    ...
    // Очищення налаштувань користувача (НОВИЙ МЕТОД)
    async clearUserSettings() {
        try {
            // Основні налаштування розкладу
            localStorage.removeItem('scheduleSettings');
            
            // Додаткові налаштування якщо є
            localStorage.removeItem('userPreferences');
            localStorage.removeItem('lastSelectedDate');
            localStorage.removeItem('scheduleViewMode');
            
            console.log('Налаштування користувача очищено');
        } catch (error) {
            console.error('Помилка очищення налаштувань користувача:', error);
            throw error;
        }
    }
